...
def load_cloud_data():
    try:
        ...
        st.error(f"雲端連線失敗，請檢查金鑰或網路設定。錯誤訊息: {str(e)}")
        return {"nutrition": [], "workout": [], "body_metrics": [], "custom_exercises": {}}
...
def save_cloud_data(nutrition_entries, workout_entries, body_entries, custom_exercises):
    try:
        gc = get_gsheet_client()
        sh = gc.open(SHEET_NAME)
